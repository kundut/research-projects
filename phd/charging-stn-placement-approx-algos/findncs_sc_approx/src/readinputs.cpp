...
void findBoundaryNodes_graph(graph_t& graph, workspace_t workspace)
{
   int x,y;
   ostringstream oss;
   int precision = 2;
   ...
   //cout << "vel count " << graph.count_vel << endl;
   ...
   //cout << "graph boundary nodes : " << count << endl;
   return;
}
...
pos_vec_t getFreeLocations (workspace_t workspace, pos_vec_t ob)
{
    position pos_tmp;
    pos_vec_t free_locs;
    unsigned int count1, count2, count=0;
    int flag;
...
void readWorkspace (workspace_t &workspace, pos_vec_t ob)
{
  ...
  if (ifp.is_open())
  {
    ...
    getline(ifp, line);
    istringstream (line) >> workspace.d;
